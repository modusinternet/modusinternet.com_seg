...
<html id="no-fouc" dir="{CCMS_LIB:_default.php;FUNC:ccms_lng_dir}" lang="{CCMS_LIB:_default.php;FUNC:ccms_lng}" style="opacity: 0;">
	<head>
		<meta charset="utf-8">
		<title dir="{CCMS_LIB:_default.php;FUNC:ccms_lng_dir}">SEG Magnetics, Inc. - {CCMS_DB:all,news} | Optin</title>
...
if($CLEAN["suDate"] == "") {
	$json['error']['suDate'] = "The 'suDate' variable is missing.<br /><br /><br />";
} elseif($CLEAN["suDate"] == "MAXLEN") {
	$json['error']['suDate'] = "The 'suDate' field exceeded its maximum size of 16 characters.<br /><br /><br />";
} elseif($CLEAN["suDate"] == "INVAL") {
	$json['error']['suDate'] = "The 'suDate' field contains invalid characters.  ( > < & # )  You have used characters in this field which are either not supported by this field or we do not permitted on this system.";
}

if(!isset($json['error'])) {
	$qry = $CFG["DBH"]->prepare("SELECT * FROM `newsletter` WHERE `email` = CONVERT(_utf8 :cuEmail USING utf8mb4) COLLATE utf8mb4_general_ci AND `suDate` = :suDate LIMIT 1;");
	$qry->execute(array(':cuEmail' => $CLEAN["cuEmail"], ':suDate' => $CLEAN["suDate"]));
	...
	if($row) {
		if($row["optout"] == "1") {
			...
			$a = time();
			$qry = $CFG["DBH"]->prepare("UPDATE `newsletter` SET `optin` = '1', `optinDate` = :optinDate WHERE `id` = :id LIMIT 1;");
			$qry->execute(array(':id' => $row["id"], ':optinDate' => $a));
		}
	} else {
